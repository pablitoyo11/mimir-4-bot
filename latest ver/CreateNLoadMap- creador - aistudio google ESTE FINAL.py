...
class WindowTrackerApp:
    ...
    def save_position(self):
        ...
        if hwnd is not None and mouse_x is not None and mouse_y is not None:
            ...
            if new_data:
                if edit_mode:
                    saved_positions[edit_index] = [self.image_path if self.image_path else " ", new_data]
                    edit_mode = False
                    edit_index = None
                else:
                    saved_positions.append([self.image_path if self.image_path else " ", new_data])
    ...
    def load_data(self):
        global saved_positions, checkbox_states
    ...
    def load_image(self):
        ...
        if file_path:
            ...
            if not saved_positions:
                saved_positions.append([self.image_path, " "])
            elif saved_positions[0][0] != self.image_path:
                saved_positions.insert(0, [self.image_path, " "])
            else:
                saved_positions[0][0] = self.image_path
            self.update_chatbox()
    ...
    def update_chatbox(self):
        ...
        self.image_canvas.delete("all")
        if saved_positions and saved_positions[0][0] != " ":
           self.load_image_on_canvas(saved_positions[0][0])
        else:
            self.image_path = None
            self.zoom_scale = 1
             
        for index, item in enumerate(saved_positions):
          if index != 0 or (index == 0 and item[0] == " "):
                ...
                data_label = tk.Label(line_frame, text=item[1], anchor="w", wraplength=350, bg=bg_color)
                ...
                if self.image_path and item[1].startswith(("UI point", "MAP")) :
                    try:
                        point_parts = item[1].split()
                        x = int(point_parts[-2])
                        y = int(point_parts[-1])
                ...
                if self.image_path and item[1].startswith(("UI area", "MAP")) :
                   try:
                     point_parts = item[1].split()
    ...
    def key_button_click(self):
        ...
        if self.current_mode.get() == "UI":
            def save_key(key):
                ...
                if edit_mode:
                    saved_positions[edit_index] = [self.image_path if self.image_path else " ", f'key {key}']
                    edit_mode = False
                    edit_index = None
                else:
                    saved_positions.append([self.image_path if self.image_path else " ", f'key {key}'])
                self.update_chatbox()
