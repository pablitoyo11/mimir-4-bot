...
class WindowTrackerApp:
    def __init__(self, root):
        ...
        self.input_mode = tk.StringVar(value="UI")
        self.image_tk = None
        ...
        # Input Mode Frame (Left Top)
        input_mode_frame = tk.Frame(main_frame)
        input_mode_frame.pack(side=tk.LEFT, padx=10, pady=10, expand=True)

         # Radio Buttons for Input Selection
        self.radio_image = Radiobutton(input_mode_frame, text="image input", variable=self.input_mode, value="IMAGE", command=self.set_image_input_mode)
        self.radio_image.pack(side=tk.TOP, anchor=tk.CENTER, pady=5)

        self.radio_other_window = Radiobutton(input_mode_frame, text="other window input", variable=self.input_mode, value="UI", command=self.set_ui_input_mode)
        self.radio_other_window.pack(side=tk.TOP, anchor=tk.CENTER, pady=5)
    ...
    def save_position(self):
        ...
        if collecting_two_points:
            ...
            if len(temp_points) == 2:
                if self.current_mode.get() == 'MAP':
                   ...
                   new_data = f'UI area {temp_points[0][0]} {temp_points[1][0]} {temp_points[0][1]} {temp_points[1][1]}'
                     ...
                     new_data = f'IMAGE area {temp_points[0][0]} {temp_points[1][0]} {temp_points[0][1]} {temp_points[1][1]}'
                else:
            ...
            if self.current_mode.get() == 'MAP':
               ...
               new_data = f'IMAGE point {int(x)} {int(y)}'
            else:
    ...
    def update_chatbox(self):
        ...
        for index, item in enumerate(saved_positions):
                ...
                if image_path and item.startswith(("UI point", "MAP", "IMAGE")) :
                    try:
                ...
                if image_path and item.startswith(("UI area", "MAP", "IMAGE")) :
                   try:
                     point_parts = item.split()
    ...
    def set_ui_input_mode(self):
         self.input_mode.set("UI")
