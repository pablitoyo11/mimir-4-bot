...
class WindowTrackerApp:
    def __init__(self, root):
        ...
        # Set a fixed width of 1060 and specific height of 580px
        new_width = 1060
        new_height = 580
        self.root.geometry(f"{new_width}x{new_height}")
        self.root.minsize(new_width, new_height) # set the minsize so it doesn't shrink more

        self.current_mode = tk.StringVar(value="UI")
        self.input_mode = tk.StringVar(value="otherWindow")
        self.image_tk = None
        self.fixed_image_width = 640
        self.fixed_image_height = 375
        self.image_loaded = False

        self.label = tk.Label(self.root, text="Initializing...", font=("Arial", 12))
        self.label.pack(pady=2)

        # Main Frame to Hold Radio, Key, and Input Frames (Left Side)
        main_frame = tk.Frame(self.root)
        main_frame.pack(side=tk.TOP, fill=tk.X, padx=10, pady=2)

        # Frame to Hold Radio Buttons, Key Button, and Map Inputs in one line
        radio_key_input_frame = tk.Frame(main_frame)
        radio_key_input_frame.pack(side=tk.TOP, fill=tk.X, expand = True)

        # Radio Buttons and Key Button Frame (Left Top, all in one line)
        radio_key_frame = tk.Frame(radio_key_input_frame)
        radio_key_frame.pack(side=tk.LEFT,  padx=2, pady = 2)


        # Radio Buttons for Mode Selection
        self.radio_ui = Radiobutton(radio_key_frame, text="UI", variable=self.current_mode, value="UI",
                                    command=self.set_ui_mode)
        self.radio_ui.pack(side=tk.LEFT, anchor=tk.CENTER, padx=2)

        self.radio_map = Radiobutton(radio_key_frame, text="MAP", variable=self.current_mode, value="MAP",
                                     command=self.set_map_mode)
        self.radio_map.pack(side=tk.LEFT, anchor=tk.CENTER, padx=2)

        # Key Button (Left Top, in the same line as radio buttons)
        self.key_button = tk.Button(radio_key_frame, text="KEY", command=self.key_button_click)
        self.key_button.pack(side=tk.LEFT, anchor=tk.CENTER, padx=2)
        
        # Input Fields Frame (Left Top), align horizontally in the same line as radio buttons and key
        input_frame = tk.Frame(radio_key_input_frame)
        input_frame.pack(side=tk.LEFT,  padx=2, pady = 2)
        
        self.map_name_entry_label = tk.Label(input_frame, text="Map Name:")
        self.map_name_entry_label.pack(side=tk.LEFT, anchor=tk.CENTER, padx=2)
        self.map_name_entry = tk.Entry(input_frame, width=15, state=tk.DISABLED)
        self.map_name_entry.pack(side=tk.LEFT, anchor=tk.CENTER, pady=2)

        self.map_action_entry_label = tk.Label(input_frame, text="Map Action:")
        self.map_action_entry_label.pack(side=tk.LEFT, anchor=tk.CENTER, padx=2)
        self.map_action_entry = tk.Entry(input_frame, width=15, state=tk.DISABLED)
        self.map_action_entry.pack(side=tk.LEFT, anchor=tk.CENTER, pady=2)
        
         # Save and Load Buttons (Left Top)
        save_load_frame = tk.Frame(main_frame)
        save_load_frame.pack(side=tk.TOP, pady=2)

        # Data Area (Right Top)
        self.data_area = tk.Frame(self.root)
        self.data_area.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True, pady=2, padx = 2)
        # Data Area should shrink
        self.data_area.pack_propagate(False)
        
        # Frame for Checkbox All and Save/Load Buttons (Right Top)
        checkbox_save_load_frame = tk.Frame(self.data_area)
        checkbox_save_load_frame.pack(side=tk.TOP, pady=2, fill=tk.X)

        # Checkbox All Button (Right Top)
        self.checkbox_all_button = tk.Button(checkbox_save_load_frame, text="Select All / None", command=self.toggle_all_checkboxes)
        self.checkbox_all_button.pack(side=tk.LEFT, pady=2, anchor=tk.CENTER)
        
        # Save and Load Buttons (Right Top, closer to the chatbox)
        self.save_button = tk.Button(checkbox_save_load_frame, text="Save", command=self.save_data)
        self.save_button.pack(side=tk.LEFT, padx=2, pady=2)
        self.load_button = tk.Button(checkbox_save_load_frame, text="Load", command=self.load_data)
        ...
        # Information Label (Left Bottom, combined)
        self.info_label = tk.Label(self.root, text="At mouse location\nControl+A to initiate area input, press again to close area and save\nControl+P to insert point", wraplength=500)
        ...
        self.load_image_button.pack(side=tk.TOP, pady=2, anchor=tk.CENTER)

         # Input Mode Frame (Right of Image)
        self.input_mode_frame = tk.Frame(self.root)
        self.input_mode_frame.pack(side=tk.TOP, pady=2, anchor = tk.CENTER)

        # Image Canvas with Border (Bottom)
        self.image_canvas_container = tk.Frame(self.root, bg = "white", width=self.fixed_image_width+2, height = self.fixed_image_height+2) #Set fixed size for container
        self.image_canvas_container.pack(side=tk.BOTTOM, fill=tk.BOTH, expand = False, pady = 2, padx = 2)
         # Adjust expand to true if the image is loaded
        if self.image_loaded:
             self.image_canvas_container.pack(side=tk.BOTTOM, fill=tk.BOTH, expand = True, pady = 2, padx = 2)

        self.image_canvas = tk.Canvas(self.image_canvas_container, bg="white", highlightthickness=0, width = self.fixed_image_width, height = self.fixed_image_height) #Set fixed size for canvas
        self.image_canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx= 1, pady = 1)
        
        # Set the fixed size to be non-shrinkable
        self.image_canvas_container.pack_propagate(False)
        ...
        self.radio_button_input.pack(side=tk.LEFT, padx=2, anchor=tk.CENTER)
        ...
        self.radio_image.pack(side=tk.LEFT, padx=2, anchor=tk.CENTER)
        ...
        if collecting_two_points:
             if len(temp_points) == 2:
                 ...
                 if self.current_mode.get() == 'MAP':
                    new_data = ['MAP', map_name, map_action, 'pointarea', int(x1), int(y1), int(x2), int(y2)]
                 elif self.current_mode.get() == 'UI':
                     new_data = ['UI', 'pointarea', int(x1), int(y1), int(x2), int(y2)]
                 elif self.input_mode.get() == 'IMAGE':
                    new_data = ['pointarea', int(x1), int(y1), int(x2), int(y2)]
                 else:
                    new_data = ['pointarea', int(x1), int(y1), int(x2), int(y2)]
                 temp_points = []
                 collecting_two_points = False
                ...
                if x is not None and y is not None:
                   if self.current_mode.get() == 'MAP':
                        new_data = ['MAP', map_name, map_action, 'pointarea', int(x), int(y), int(x), int(y)]
                   elif self.current_mode.get() == 'UI':
                        new_data = ['UI', 'pointarea', int(x), int(y), int(x), int(y)]
                   else:
                        new_data = ['pointarea', int(x), int(y), int(x), int(y)]
    ...
    def load_image(self):
        ...
        if file_path:
            ...
            self.update_chatbox()
            # Redraw pointarea on canvas for all loaded elements
            for index, item in enumerate(saved_positions):
                if image_path and isinstance(item, list) and item[0] in ("UI", "MAP", "pointarea"):
                    try:
                        x1 = int(item[-4])
                        y1 = int(item[-3])
                        x2 = int(item[-2])
                        y2 = int(item[-1])
                        self.draw_pointarea(x1, y1, x2, y2, index, checkbox_states.get(index, True))
                    except Exception as e:
    ...
    def load_image_on_canvas(self, image_path):
         ...
         try:
            ...
            canvas_width = self.fixed_image_width
            canvas_height = self.fixed_image_height
    ...
    def update_window_size(self, image_width, image_height):
        min_width = 1060  # set initial min width
        min_height = 580  # set initial min height
    ...
    def update_chatbox(self, current_item=None):
        # Destroy existing widgets in the chat_frame
        for widget in self.chat_frame.winfo_children():
            widget.destroy()

        # Clear the image canvas and load the image if you are not updating one single item
        if not current_item:
            self.image_canvas.delete("all")
            if image_path:
                self.load_image_on_canvas(image_path)
        ...
        for index, item in enumerate(saved_positions):
            ...
            line_frame.pack(fill=tk.X, padx=2, pady=1)
            ...
            data_label.grid(row=0, column=2, sticky="ew", padx=2)
    ...
    def draw_pointarea(self, x1, y1, x2, y2, line_number, is_checked):
        rectangle_tag = f"rectangle_{line_number}"
        point_tag = f"point_{line_number}"
        text_tag = f"text_{line_number}"

        if not is_checked:
            self.image_canvas.itemconfig(rectangle_tag, state="hidden")
            self.image_canvas.itemconfig(point_tag, state="hidden")
            self.image_canvas.itemconfig(text_tag, state="hidden")
            return
        else:
            self.image_canvas.itemconfig(rectangle_tag, state="normal")
            self.image_canvas.itemconfig(point_tag, state="normal")
            self.image_canvas.itemconfig(text_tag, state="normal")
        ...
        # Move or create rectangle
        if not self.image_canvas.find_withtag(rectangle_tag):
            self.image_canvas.create_rectangle(canvas_x1, canvas_y1, canvas_x2, canvas_y2, outline="green", tags=rectangle_tag)
        else:
            self.image_canvas.coords(rectangle_tag, canvas_x1, canvas_y1, canvas_x2, canvas_y2)

        #Move or create Point
        radius = 3
        point_x = canvas_x1
        point_y = canvas_y1
        if not self.image_canvas.find_withtag(point_tag):
             self.image_canvas.create_oval(point_x - radius, point_y - radius, point_x + radius, point_y + radius, fill="red",
                                         tags=point_tag)
        else:
            self.image_canvas.coords(point_tag, point_x - radius, point_y - radius, point_x + radius, point_y + radius)
        # Move or create Text
        if not self.image_canvas.find_withtag(text_tag):
             self.image_canvas.create_text(point_x + 8, point_y, text=f"{line_number}", fill="blue", anchor=tk.W,
                                      tags=text_tag)
        else:
            self.image_canvas.coords(text_tag, point_x + 8, point_y)

    def delete_line(self, index):
        global saved_positions
        if image_path:
            rectangle_tag = f"rectangle_{index}"
            point_tag = f"point_{index}"
            text_tag = f"text_{index}"
            self.image_canvas.delete(rectangle_tag)
            self.image_canvas.delete(point_tag)
            self.image_canvas.delete(text_tag)
    ...
    def toggle_checkbox(self, index, checkbox_var):
        ...
        #Update on canvas only if image exist
        if image_path:
            item = saved_positions[index]
            if isinstance(item, list) and item[0] in ("UI", "MAP", "pointarea"):
                try:
                    x1 = int(item[-4])
                    y1 = int(item[-3])
                    x2 = int(item[-2])
            ...
            if image_path:
                ...
                if isinstance(item, list) and item[0] in ("UI", "MAP", "pointarea"):
                    try:
                        ...
                        x2 = int(item[-2])
                        y2 = int(item[-1])
                        self.draw_pointarea(x1, y1, x2, y2, index, new_state)
                    except Exception as e:
                        print(f"error reading item pointarea in chatbox{e}")
    ...
    def key_button_click(self):
        ...
        if self.current_mode.get() == "UI":
            def save_key(key):
                ...
                if edit_mode:
                    saved_positions[edit_index] = ['key', key]
                    edit_mode = False
                    edit_index = None
                else:
                    saved_positions.append(['key', key])
                self.update_chatbox()
