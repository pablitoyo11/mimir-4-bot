...
def capture_window_screenshot(window_title):
    ...
    cropped_image.save(f"tmpscreens/{window_title}.png")
    


def save_part_of_image(window_title, x_percent, y_percent, width_percent, height_percent, output_file):
    # Load the image
    image = Image.open(f"tmpscreens/{window_title}.png")
    img_width, img_height = image.size
    
    try:
        # Create folder for screenshots
        folder_path = (f"./tmpscreens/{window_title}")
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
    except Exception as error:
        print(f"Error creating folder")
    ...
    cropped_image.save(f"tmpscreens/{window_title}/{output_file}.png")    # Save
##    print(f"{output_file} Part of the image has been saved to {window_title}_{output_file}.png")
