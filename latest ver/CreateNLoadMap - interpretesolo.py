...
import os
import random
from ClickAtMir import get_window_click_map_randomizer, get_window_click_space_randomizer

def interprete_de_accion(linea, window_title):
    try:
        duration = random.randint(2, 3)
        if len(linea) == 3:
            x, y, action_type = linea
            print(f"action_type action_type      {action_type}")
            get_window_click_map_randomizer(x, y, window_title, duration)
            if "Handler" in action_type:
                get_window_click_map_randomizer(x, y, window_title, duration)
                get_window_click_map_randomizer(x, y, window_title, duration)
        elif len(linea) == 5:
            xs, ys, xe, ye, action_type = linea
            print(f"action_type action_type      {action_type}")
            get_window_click_space_randomizer(xs, ys, xe, ye, window_title, duration)
        return action_type
...
def travel_to_map(intended_map, window_title):
    archivo = f"secuencias/maptravel/{intended_map}"
    try:
...
def manejar_area(archivo, xs=None, ys=None, xe=None, ye=None, accion=None):
    try:
        # Solicitar los valores al usuario solo si no se proporcionan como parámetros
        if xs is None:
        ...
        if accion is None:
            ...
            print("4. RecolectingHandler")
            print("5. MiningDarkSteelHandler")
            print("6. RecolectingVoidOrbHandler")
            print("7. AtackHandler")
            print("8. MovingWaitHandler")
            accion_opcion = input("Ingrese el número de la acción: ").strip()
            while accion_opcion not in ['1', '2', '3', '4', '5', '6', '7', '8']:
                accion_opcion = input("Respuesta inválida. Ingrese el número de la acción (1, 2, 3, 4, 5, 6, 7 u 8): ").strip()
            if accion_opcion == '1':
                ...
                accion = 'RecolectingHandler'
            elif accion_opcion == '5':
                accion = 'MiningDarkSteelHandler'
            elif accion_opcion == '6':
                accion = 'RecolectingVoidOrbHandler'
            elif accion_opcion == '7':
                accion = 'AtackHandler'
            elif accion_opcion == '8':
                ...
                if os.path.exists(archivo):
                    ...
                    if accion_mapa == '1':
                        ...
                        if editar_opcion == '1':
                            ...
                            mostrar_acciones(archivo)  # Mostrar las acciones después de editar
                            break  # Salir del bucle después de editar
                            ...
                            try:
                                ...
                                print("3. ClickPunto")
                                accion_opcion = input("Ingrese el número de la acción: ").strip()
                                while accion_opcion not in ['1', '2', '3']:
                                    accion_opcion = input("Respuesta inválida. Ingrese el número de la acción (1, 2 o 3): ").strip()
                                if accion_opcion == '1':
                                    # Generar automáticamente la línea de AbrirMapa
                                    manejar_area(archivo, 520, 71, 590, 84, 'AbrirMapa')
                                    manejar_click_mapa(archivo)
                                elif accion_opcion == '2':
                                    manejar_area(archivo)  # Llamar a manejar_area sin repetir inputs
                                elif accion_opcion == '3':
                                    manejar_click_mapa(archivo)
                            except ValueError:
                                print("Entrada inválida. Por favor, ingrese números enteros.")
